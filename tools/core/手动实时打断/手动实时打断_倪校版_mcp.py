#!/usr/bin/env python3
"""
FractFlow 手动实时打断语音控制 - 倪校版MCP服务器
基于千问Omni实时语音API + 倪校TTS音色，支持立即打断AI回答功能
"""
...
# 导入手动实时打断Agent（倪校版）
from tools.core.手动实时打断.手动实时打断_倪校版_agent import SimpleManualVoiceController
...
async def handle_list_tools() -> list[types.Tool]:
    ...
    return [
        types.Tool(
            ...
            description="启动倪校版手动实时打断语音控制助手（倪校TTS音色）",
            inputSchema={
                "type": "object",
                "properties": {
                    "enable_context": {
...
async def handle_call_tool(
    ...
    try:
        if name == "start_manual_interrupt_voice_control":
            ...
            status_text = f"""✅ 倪校版手动实时打断语音控制助手已启动！
🎓 倪校长音色播放模式已激活

📋 操作说明：
   • 使用 start_recording 开始录音
   • 使用 stop_recording_and_send 停止录音并发送  
   • 使用 interrupt_ai_response 立即打断倪校长回答
...
🎓 倪校长状态: {'🟢 正在回答' if voice_controller.is_ai_speaking else '⚪ 空闲'}
