#!/usr/bin/env python3
"""
FractFlow 手动语音控制工具 - 倪校音色版
基于千问Omni实时语音API + 倪校音色TTS + 终端回车控制（无需管理员权限）
"""
...
class NiTTSPlayer:
    ...
    def __init__(self):
        self.tts_url = "http://10.120.17.57:9880/tts"
    ...
    def initialize_audio(self):
        ...
        try:
            ...
            print(f"❌ 倪校TTS音频初始化失败: {e}")
            return False
    
    def play_text(self, text):
        """播放倪校音色的文本 - 优化版，支持立即打断"""
        if not text.strip():
            return
        ...
        try:
            ...
            try:
                ...
                with session.get(
                    ...
                    # 使用更小的chunk_size以提高响应速度
                    for chunk in response.iter_content(chunk_size=256):  # 从1024减小到256
                        # 立即检查停止信号 - 多重检查
                        if self.stop_flag or self.stop_event.is_set():
                            print("🔇 倪校TTS收到停止信号，立即退出")
                            break
                        ...
                        if chunk:
                            ...
                            # 尝试解析WAV头并开始播放
                            if self.stream is None and len(buffer) > 44:
                                try:
                                    # 再次检查停止信号
                                    if self.stop_flag or self.stop_event.is_set():
                                        break
                                    ...
                                    with self.stream_lock:
                                        if not (self.stop_flag or self.stop_event.is_set()):
                                            self.stream = self.pyaudio_instance.open(
                                                format=self.pyaudio_instance.get_format_from_width(wav.getsampwidth()),
                                                channels=wav.getnchannels(),
                                                rate=wav.getframerate(),
                                                output=True,
                                                frames_per_buffer=256  # 小缓冲区
                                            )
                                    
                                    # 播放WAV头后的数据
                                    wav_io.seek(44)
                                    header_data = wav_io.read()
                                    if header_data and not (self.stop_flag or self.stop_event.is_set()):
                                        self._write_audio_chunk(header_data)
                        ...
                        # 每10个chunk检查一次停止信号，提高响应速度
                        if chunk_count % 10 == 0:
                            if self.stop_flag or self.stop_event.is_set():
                                print("🔇 倪校TTS定期检查发现停止信号")
                                break
            ...
            self.cleanup()
            # 播放完成，通知主控制器
            if self.on_playback_finished and not (self.stop_flag or self.stop_event.is_set()):
                try:
                    self.on_playback_finished()
                except Exception as e:
                    print(f"⚠️ 播放完成回调错误: {e}")
    
    def _write_audio_chunk(self, chunk):
        """安全写入音频块，支持立即打断"""
        try:
            with self.stream_lock:
                if self.stream and not (self.stop_flag or self.stop_event.is_set()):
                    # 将大块分解为小块，提高打断响应速度
                    ...
                    for i in range(0, len(chunk), chunk_size):
                        ...
                        if self.stream and self.stream.is_active():
                            self.stream.write(mini_chunk, exception_on_underflow=False)
        except Exception as e:
            # 忽略播放错误，可能是被打断导致的
            pass
    ...
    def stop(self):
        ...
        # 第二步：立即关闭音频流，阻止新音频输出
        self._force_stop_audio_stream()
        
        # 第三步：尝试终止HTTP请求
        self._force_stop_http_request()
        
        # 第四步：快速等待播放线程结束
        if self.play_thread and self.play_thread.is_alive():
            self.play_thread.join(timeout=0.1)  # 100ms超时
            if self.play_thread.is_alive():
                print("⚠️ 倪校TTS播放线程未能在100ms内结束")
        
        # 第五步：强制清理所有资源
        self.cleanup()
        print("🔇 倪校TTS已立即停止")
    
    def _force_stop_audio_stream(self):
        """强制立即停止音频流"""
        try:
            with self.stream_lock:
                if self.stream:
            ...
            print(f"⚠️ 强制停止音频流时出错: {e}")
    
    def _force_stop_http_request(self):
        """强制终止HTTP请求"""
        try:
            if self.http_session:
                self.http_session.close()
                self.http_session = None
                print("🔇 倪校TTS HTTP会话已关闭")
        except Exception as e:
            print(f"⚠️ 关闭HTTP会话时出错: {e}")
...
class SimpleManualVoiceController:
    ...
    def __init__(self):
        ...
        # 初始化倪校TTS播放器
        self.ni_tts = NiTTSPlayer()
        # 设置TTS播放完成回调
        self.ni_tts.on_playback_finished = self._on_tts_finished
        print("🎓 倪校TTS播放器已初始化")
    ...
    def setup_audio(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 初始化倪校TTS
            if not self.ni_tts.initialize_audio():
                return {"success": False, "error": "倪校TTS音频初始化失败"}
            
            print("🔊 线程安全音频系统初始化完成")
            print("🎓 倪校TTS音频系统初始化完成")
            return {
                "success": True,
                "message": "线程安全音频系统和倪校TTS已初始化"
            }
        except Exception as e:
            return {
                "success": False,
    ...
    async def _handle_response_event(self, response_type: str, data: Dict[str, Any]):
        ...
        if response_type == "conversation.item.input_audio_transcription.completed":
            ...
            if self.current_response.strip():
                print()  # 换行
                # 使用倪校TTS播放完整回答
                if self.is_ai_speaking:  # 确保没有被打断
                    # 启动TTS播放，播放期间AI应该保持说话状态
                    self.ni_tts.play_text(self.current_response.strip())
                    # 注意：不要在这里设置is_ai_speaking=False，要等TTS播放完成
        
        elif response_type == "response.audio.delta":
            # AI音频回答增量 - 忽略，使用倪校TTS代替
            # 在倪校版本中，我们不播放原始音频，而是等待文本完成后使用TTS
            pass
        
        elif response_type == "response.done":
            # AI文本回答完成，但TTS可能还在播放
            # 只保存对话，不设置is_ai_speaking=False（由TTS完成回调设置）
    ...
    def _cleanup(self):
        ...
        # 清理倪校TTS
        if hasattr(self, 'ni_tts'):
            try:
                self.ni_tts.stop()
                self.ni_tts.cleanup()
                print("🎓 倪校TTS已清理")
            except Exception as e:
                print(f"⚠️ 倪校TTS清理错误: {e}")
    ...
    async def run_interactive_mode(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print("   3. 倪校长回答时按回车键打断并0.3秒后开始录音 ⚡")
...
async def start_simple_manual_voice_control() -> str:
    ...
    基于千问Omni实时语音API + 倪校TTS音色，使用终端回车键控制（无需管理员权限）。
    ...
    try:
        ...
        if result["success"]:
            return f"✅ 倪校版手动语音控制助手已启动并运行完成。{result.get('message', '')}"
        else:
            return f"❌ 启动失败: {result['error']}"
            
    except Exception as e:
        return f"❌ 启动倪校版手动语音控制助手失败: {str(e)}"

# 直接运行模式
async def main():
    """直接运行倪校版手动语音控制助手"""
    print("🎤 FractFlow 倪校版手动语音控制助手")
    print("=" * 60)
    print("🎯 基于千问Omni实时语音API + 倪校TTS音色 + 终端回车控制")
    print("⚡ 特色：无需管理员权限，终端输入控制，倪校长音色播放")
    print("=" * 60)
